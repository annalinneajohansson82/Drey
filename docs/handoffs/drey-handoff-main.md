...
- **Minimal data model drafted and committed.** See `docs/plans/data-model.md`. Defines: Item entity with four states (`living`, `dormant`, `finished`, `closed`), `PutDownNote` structured object (four optional fields: `left_off`, `next_step`, `draw`, `reminder`), `AppetiteTags` (`energy`, `engagement` — slider input, taxonomy TBD), close note (persists after close; feeds pattern view), and local-first JSON storage. Four open questions remain in the doc: appetite tag taxonomy, put-down note history, finished-state behavior, and state naming.
...
- **Close note coupling:** the close prompt ("Any final thoughts on this before it's closed?") and the pattern view evolve together; the close note must be visibly retained at the moment of closing so the prompt never has to explain itself.
...
- **Living / dormant items** = active vs. set-down. **Finished** = opt-in, separate area. **Closed** = done handling; its close note feeds the pattern view.
- **Pattern view** = on-request-only mirror of commonalities across closed items; surfaces, never concludes.
